...
const CricGully = () => {
  ...
  const loadMatchHistory = async () => {
    try {
      const history = await AsyncStorage.getItem('gullyMatchHistory');
      if (history) {
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              await AsyncStorage.removeItem('gullyMatchHistory');
              setMatchHistory([]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push({
        pathname: '/gullyMatchScreen',
        params: {
          teamA: team1,
          teamB: team2,
          matchNumber: 'Single',
          isSingleMatch: true,
          teamAPlayers: JSON.stringify(team1Players.filter(p => p)),
          teamBPlayers: JSON.stringify(team2Players.filter(p => p))
        },
      });
    } catch (error) {
