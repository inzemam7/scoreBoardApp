...
const SingleMatchScreen = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch current match data from AsyncStorage
                const currentMatchData = await AsyncStorage.getItem('currentMatchData');
                if (currentMatchData) {
                    const parsedData = JSON.parse(currentMatchData);
                    setOversLimit(parsedData.overs);
                    setTeamAPlayers(parsedData.team1.players);
                    setTeamBPlayers(parsedData.team2.players);
                }
    ...
    const updateBall = (runs, extra = false, isWicket = false, extraType = null) => {
        ...
        if (isWicket) {
            ...
            if (newData[inningIndex].wickets + 1 >= maxWickets) {
                ...
                // Handle match progression after all out
                if (currentInning === 1) {
                    // First innings all out - start second innings
                    Alert.alert(
                        'Team All Out',
                        `${battingTeam} has been all out. ${bowlingTeam} will now bat.`,
                        [{ text: 'OK', onPress: () => startSecondInnings(true) }]
                    );
                } else {
                    // Second innings all out - end match
                    ...
                    Alert.alert(
                        'Team All Out',
                        `${battingTeam} has been all out. ${bowlingTeam} wins the match!`,
                        [{ text: 'OK', onPress: () => endMatch(matchWinner) }]
                    );
                }
                return;
            }
        ...
        if (isOversCompleted || isAllOut) {
            ...
            if (currentInning === 1) {
                ...
                Alert.alert(
                    isAllOut ? 'Team All Out' : 'Overs Completed',
                    isAllOut 
                        ? `${battingTeam} has been all out. ${bowlingTeam} will now bat.`
                        : `First innings completed. ${bowlingTeam} will now bat.`,
                    [{ text: 'OK', onPress: () => startSecondInnings(isAllOut) }]
                );
            } else {
                // Second innings completed - end match
                const team1Score = inningsData[0].score;
                const team2Score = newData[1].score;
                const matchWinner = team2Score > team1Score ? battingTeam : team2Score < team1Score ? bowlingTeam : 'Tie';
                Alert.alert(
                    isAllOut ? 'Team All Out' : 'Overs Completed',
                    isAllOut 
                        ? `${battingTeam} has been all out. ${matchWinner} wins the match!`
                        : `Match completed. ${matchWinner} wins the match!`,
                    [{ text: 'OK', onPress: () => endMatch(matchWinner) }]
                );
            }
            return;
        ...
        <ScrollView style={styles.container}>
            <View style={styles.header}>
                <Text style={styles.title}>
                    {isViewOnly ? '📜 Match Details' : '🏏 Single Match'}
                </Text>
